...
> [!Note] 
> ```latest``` tagged image is maybe a pre- or development-release. Please use always a release number (like ```3.11.1.1```) 
...
```
make build PI_VERSION=3.11.1 PI_VERSION_BUILD=3.11.1
```
...
---------|----------|---|---------
| ```build ``` | ```PI_VERSION```<br> ```IMAGE_NAME```|Build an image. Optional: specify the version, requirements version and image name| ```make build PI_VERSION=3.11.1 PI_VERSION_BUILD=3.11.1```|
...
```
make cert build push stack PI_VERSION=3.11.1 PI_VERSION_BUILD=3.11.1 TAG=pidev
```
---
