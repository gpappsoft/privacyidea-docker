...
> [!Note] 
> ```latest``` tagged image is maybe a pre- or development-release. Please use always a release number (like ```3.13dev3```) 
...
```
make build PI_VERSION=3.13dev3 PI_VERSION_BUILD=3.13dev3
```
...
---------|----------|---|---------
| ```build ``` | ```PI_VERSION```<br> ```IMAGE_NAME```|Build an image. Optional: specify the version, requirements version and image name| ```make build PI_VERSION=3.13dev3 PI_VERSION_BUILD=3.13dev3```|
...
```
make cert build push stack PI_VERSION=3.13dev3 PI_VERSION_BUILD=3.13dev3 TAG=pidev
```
---
